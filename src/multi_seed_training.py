...
from utils.callbacks import create_evaluation_config, MetricTracker, create_model_checkpoint_callback
...
def train_single_seed(cfg: DictConfig, seed: int, extra_name: str) -> Tuple[torch.Tensor, Dict, MetricTracker, float, float]:
    """
    ...
    try:
        ...
        final_coverage = metrics_history["coverage"][-1] if metrics_history["coverage"] else 0.0
        ...
        if not np.isfinite(final_train_loss) or not np.isfinite(final_coverage):
            ...
            last_finite_coverage_idx = np.where(np.isfinite(metrics_history["coverage"]))[0]
...
def main_multi_seed(cfg: DictConfig):
    ...
    for seed in seeds:
        samples, metrics_history, tracker, train_loss, coverage = train_single_seed(cfg, seed, extra_name)
    ...
    if (cfg.main.dataset.lower() == "two_moons" or 
        cfg.main.dataset.lower() == "2d_gaussians"):
    ...
    for seed in seeds:
        if seed in all_train_losses:
            log.info(f"Seed {seed:3d}: Train Loss = {all_train_losses[seed]:.4f}, "
