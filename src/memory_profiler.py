...
class ModelMemoryProfiler:
    def __init__(self, model_class, input_shape: Tuple[int, ...]):
        ...
        self.results = []
        self.num_timesteps = 1000  # Default number of timesteps, can be adjusted as needed
    ...
    def get_memory_usage(self) -> Dict[str, float]:
        ...
        if torch.cuda.is_available():
            memory_info['gpu_allocated_torch'] = torch.cuda.memory_allocated() / 1024**2  # MB
            memory_info['gpu_reserved_torch'] = torch.cuda.memory_reserved() / 1024**2   # MB
            memory_info['gpu_max_allocated_torch'] = torch.cuda.max_memory_allocated() / 1024**2  # MB
    ...
    def test_configuration(self, hidden_dim: int, num_layers: int, batch_size: int) -> Dict:
        """
        ...
        self.input_shape = (batch_size, 3, 256, 256)  # Update input shape based on batch size
        ...
        try:
            ...
            dummy_time = torch.randint(0, self.num_timesteps, (self.input_shape[0],), device=self.device)
            dummy_noise = torch.randn_like(dummy_input, device=self.device)  # Assuming x_start is defined in your model
            ...
            if torch.cuda.is_available():
                result['gpu_allocated_baseline'] = baseline_memory.get('gpu_allocated_torch', 0)
                result['gpu_allocated_after_creation'] = memory_after_creation.get('gpu_allocated_torch', 0)
                result['gpu_allocated_after_device'] = memory_after_device.get('gpu_allocated_torch', 0)
                result['gpu_allocated_after_input'] = memory_after_input.get('gpu_allocated_torch', 0)
                result['gpu_allocated_after_forward'] = memory_after_forward.get('gpu_allocated_torch', 0)
                
                # Memory deltas
                result['memory_delta_model_creation'] = memory_after_creation.get('gpu_allocated_torch', 0) - baseline_memory.get('gpu_allocated_torch', 0)
                result['memory_delta_to_device'] = memory_after_device.get('gpu_allocated_torch', 0) - memory_after_creation.get('gpu_allocated_torch', 0)
                result['memory_delta_input'] = memory_after_input.get('gpu_allocated_torch', 0) - memory_after_device.get('gpu_allocated_torch', 0)
                result['memory_delta_forward'] = memory_after_forward.get('gpu_allocated_torch', 0) - memory_after_input.get('gpu_allocated_torch', 0)

                result['gpu_memory_used_torch'] = memory_after['gpu_allocated_torch'] - memory_before.get('gpu_allocated_torch', 0)
                result['gpu_peak_memory_torch'] = memory_after['gpu_max_allocated_torch']
                result['gpu_reserved_torch'] = memory_after['gpu_reserved_torch']

                if 'gpu_used_system' in memory_after:
                    result['gpu_memory_used_system'] = memory_after['gpu_used_system'] - memory_before.get('gpu_used_system', 0)
                    ...
                    result['system_memory_baseline'] = baseline_memory.get('gpu_used_system', 0)
                    result['system_memory_after_device'] = memory_after_device.get('gpu_used_system', 0)
                    result['system_memory_after_forward'] = memory_after_forward.get('gpu_used_system', 0)
                else:
            ...
            result['output_shape'] = str(output.shape) if hasattr(output, 'shape') else 'N/A'
            ...
            if len(self.results) < 3:  # Only for first 3 configurations
                ...
                print(f"    Model params: {total_params:,} ({result['model_size_mb']:.1f} MB)")
                if torch.cuda.is_available():
                    print(f"    PyTorch GPU allocated: {result['gpu_allocated_baseline']:.1f} → {result['gpu_allocated_after_forward']:.1f} MB")
                    print(f"    Memory deltas: creation={result['memory_delta_model_creation']:.1f}, "
                    ...
                    if result['gpu_memory_used_system'] != 'N/A':
                        print(f"    System GPU: {result['system_memory_baseline']:.1f} → {result['system_memory_after_forward']:.1f} MB")
    ...
    def _set_error_defaults(self, result: Dict):
        ...
        error_fields = [
            'model_parameters', 'trainable_parameters', 'model_size_mb',
            'gpu_memory_used_torch', 'gpu_peak_memory_torch', 'gpu_reserved_torch',
            'gpu_memory_used_system', 'gpu_total_system', 'gpu_util_percent',
            'cpu_memory', 'output_shape', 'memory_delta_model_creation',
    ...
    def run_parameter_sweep(self, 
        ...
        for i, (hidden_dim, num_layers, batch_size) in enumerate(
            ...
            print(f"Progress: {i+1}/{total_configs} - Testing config: "
            ...
            if result['status'] == 'cuda_oom':
                ...
                if memory_display != 'N/A':
                    ...
                    if result.get('gpu_memory_used_system', 'N/A') != 'N/A':
                        print(f"    System GPU Memory: {result['gpu_memory_used_system']:.1f} MB")
                else:
                    print(f"  ✅ Success")
            else:
    ...
    def plot_results(self, df: pd.DataFrame, save_path: Optional[str] = None):
        ...
        if len(success_df) > 0:
            ...
            success_df['gpu_memory_torch_numeric'] = pd.to_numeric(success_df['gpu_memory_used_torch'], errors='coerce')
            success_df['model_params_numeric'] = pd.to_numeric(success_df['model_parameters'], errors='coerce')
            
            # Use system memory if available, otherwise torch memory
            if 'gpu_memory_used_system' in success_df.columns:
                success_df['gpu_memory_system_numeric'] = pd.to_numeric(success_df['gpu_memory_used_system'], errors='coerce')
        ...
        if len(success_df) > 0:
            ...
            ax2.text(0.5, 0.5, 'No successful runs', ha='center', va='center', transform=ax2.transAxes)
        ...
        if len(success_df) > 0:
            ...
            ax3.text(0.5, 0.5, 'No successful runs', ha='center', va='center', transform=ax3.transAxes)
    ...
    def print_summary(self, df: pd.DataFrame):
        ...
        print(f"Other errors: {other_errors} ({other_errors/total_configs*100:.1f}%)")
        ...
        if successful > 0:
            ...
            memory_values = pd.to_numeric(success_df[memory_col], errors='coerce').dropna()
            ...
            if len(memory_values) > 0:
                ...
                if 'gpu_memory_used_system' in success_df.columns and 'gpu_memory_used_torch' in success_df.columns:
                    torch_memory = pd.to_numeric(success_df['gpu_memory_used_torch'], errors='coerce').dropna()
                    system_memory = pd.to_numeric(success_df['gpu_memory_used_system'], errors='coerce').dropna()
                    if len(torch_memory) > 0 and len(system_memory) > 0:
                        ...
                        print(f"  Difference: {abs(system_memory.mean() - torch_memory.mean()):.1f} MB")
